...
\subsection{Paramétrage manuel des axes}

\subsubsection{Ajout de valeurs manuellement}\label{ajoutvals}
...
\subsubsection{Ajout de labels manuellement}\label{ajoutlabels}

Il est également possible d'utiliser une commande spécifique pour placer des labels sur les axes, dans le cadre de statistiques qualitatives notamment.

\begin{tcblisting}{listing engine=minted,minted language=latex,colframe=lightgray,colback=lightgray!5,listing only}
%dans l'environnement GraphiqueTikz
\RajouterLabelsAxeX[clés]{positions}{labels}
\RajouterLabelsAxeY[clés]{positions}{labels}

% ----- [clé spécifique]
% Rotation := angle pour les labels
\end{tcblisting}

\begin{tcblisting}{listing engine=minted,minted language=latex,colframe=lightgray,colback=lightgray!5}
\begin{GraphiqueTikz}%
    [x=1.2cm,y=0.4cm,Xmin=0,Xmax=7,Xgrille=1,
    Ymin=0,Ymax=15,Ygrille=5,Ygrilles=1]
  \TracerAxesGrilles[Elargir=2.5mm,Police=\small]{}{0,5,...,15}
  \RajouterLabelsAxeX[Rotation=45]%
    {1,2,3,4,5,6}%
    {Maths,Anglais,Français,Allemand,SVT,EPS}
\end{GraphiqueTikz}
\end{tcblisting}
...
\begin{itemize}
  \item \MontreCode{Swap=TF} pour colorier la partie non-solution (\MontreCode{false} par défaut) ;
...
\begin{tcblisting}{listing engine=minted,minted language=latex,colframe=lightgray,colback=lightgray!5}
\begin{GraphiqueTikz}[x=0.4cm,y=0.4cm,Xmin=-4,Xmax=6,Ymin=-3,Ymax=7]
...
\begin{GraphiqueTikz}[x=0.4cm,y=0.4cm,Xmin=-4,Xmax=6,Ymin=-3,Ymax=7]
  \TracerAxesGrilles[Police=\tiny]{auto}{auto}
  \InegaliteLineaire[Swap,Style=remplissage,Couleur=teal]{-x-y-4}{>0}
\end{GraphiqueTikz}
\end{tcblisting}
...
\begin{description}[noitemsep]
  ...
  \lstcmd{aj val axe X~~~}{\RajouterValeursAxeX}{ajoutvals}
  \lstcmd{aj val axe Y~~~}{\RajouterValeursAxeY}{ajoutvals}
  \lstcmd{aj labels axe X}{\RajouterLabelsAxeX}{ajoutlabels}
  \lstcmd{aj labels axe Y}{\RajouterLabelsAxeY}{ajoutlabels}
...
\begin{verbatim}
0.31a : Environnement alt pour des valeurs non normalisées + bugfixes InegaliteLin + Labels pour axes
